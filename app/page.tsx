...
/* ─── User avatar – actual brand icon. Gold glow for admin, accent for users ─── */
function UserAvatarIcon({ expanded, role }: { expanded: boolean; role?: "admin" | "user" }) {
  ...
  const gradientId = isAdmin ? "avatar-stroke-admin" : expanded ? "avatar-stroke-in" : "avatar-stroke-out";
  return (
    <svg
          ...
          <>
            ...
            >
              ...
              <div className="shrink-0 p-4 border-t border-[#e5e5e5]">
                <button
                  onClick={() => { setMobileSidebarOpen(false); handleAvatarClick(); }}
