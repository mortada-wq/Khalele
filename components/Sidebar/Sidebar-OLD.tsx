...
export interface Contact {
  id: string;
  name: string;
  code: string; // سرخليل code like &mg1
  avatar?: string;
}
...
export interface SidebarProps {
  ...
  onSearchDiwan?: (query: string) => void;
...
export function Sidebar({
  ...
  const [diwanSearchQuery, setDiwanSearchQuery] = useState("");
  const [directorySearchQuery, setDirectorySearchQuery] = useState("");
  const [casesSearchQuery, setCasesSearchQuery] = useState("");
  const searchDebounce = useRef<ReturnType<typeof setTimeout>>();

  const handleDiwanSearch = useCallback(
    (q: string) => {
      setDiwanSearchQuery(q);
      if (searchDebounce.current) clearTimeout(searchDebounce.current);
      searchDebounce.current = setTimeout(() => {
        onSearchDiwan?.(q.trim());
      }, 300);
    },
    [onSearchDiwan]
  );
  ...
  const handleDirectorySearch = useCallback(
    (q: string) => {
      setDirectorySearchQuery(q);
      if (searchDebounce.current) clearTimeout(searchDebounce.current);
      searchDebounce.current = setTimeout(() => {
        onSearchDirectory?.(q.trim());
      }, 300);
    },
    [onSearchDirectory]
  );
  ...
  const handleCasesSearch = useCallback(
    (q: string) => {
      setCasesSearchQuery(q);
      if (searchDebounce.current) clearTimeout(searchDebounce.current);
      searchDebounce.current = setTimeout(() => {
        onSearchCases?.(q.trim());
      }, 300);
    },
    [onSearchCases]
  );
  ...
  const sidebarContent = (isMobile: boolean) => (
    <>
      ...
      >
        {onCreateDiwan && (
          <button
            ...
            className="w-full flex items-center gap-1.5 px-3 py-2 rounded-lg font-ui text-xs transition-colors hover:bg-black/5 dark:hover:bg-white/5 mb-2"
        ...
        <div className="mb-3">
          <input
            ...
            value={diwanSearchQuery}
            onChange={(e) => handleDiwanSearch(e.target.value)}
      ...
      {/* Part 2.5: دليل (Directory/Contacts) */}
      <SidebarSection
        icon={<IconDirectory />}
        label="دليل"
        expanded={isMobile || expanded}
      ...
      >
        <div className="mb-3">
          <input
            ...
            onChange={(e) => handleCasesSearch(e.target.value)}
            className="w-full px-3 py-2 rounded-xl font-ui text-xs"
            style={{
              background: "var(--bg-tertiary)",
              border: "var(--border-subtle)",
    ...
    <>
        ...
        <div className="shrink-0 flex items-center justify-center" style={{ height: 52 }}>
          {onToggleSidebar && (
            <button
              ...
              className="relative flex items-center justify-center p-1 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
          ...
          {expanded ? (
            ...
            <div className="flex-1 flex flex-col items-center gap-1 pt-2">
              ...
              <button
                ...
                title="دليل"
                style={{
                  color: activeSection === "directory" ? "var(--color-accent)" : "var(--text-tertiary)",
                }}
              >
                <IconDirectory />
              </button>
      ...
      {onToggleSidebar && !expanded && (
        <button
          type="button"
          onClick={onToggleSidebar}
          className="md:hidden fixed top-4 z-50 relative flex items-center justify-center p-3 rounded-xl bg-transparent border-none shadow-none hover:opacity-80 active:opacity-70 transition-opacity"
          style={{ right: 12 }}
